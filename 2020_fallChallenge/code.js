...
class Player {
  ...
  setSimul() {
    ...
    sit.spellBook.setAllSpellCastable();
    // 2. Le resultat est la potion à cibler
    const result = [];
    ...
    loopWhile: while (!queue.isEmpty()) {
      ...
      queue.dequeue();
      // On regarde si tous les sorts sont utilisés
      // const spells = sit.spellBook.list.map(e => {return e.castable})
      // if(!spells.include(1)) {
      //   sit.spellBook.
      // }
      ...
      loopSpell: for (let i = 0; i < sit.spellBook.list.length; i++) {
        ...
        const spell = newSpellBook.list[i];
  ...
  setAction() {
    const shopIdList = Merch.list.map((e) => {
      return e.id;
    });
    // On vérifie que la potion ciblée existe toujours, si non on resimule pour refaire un path
    if (!this.simulation && !shopIdList.includes(this.pathTarget.id)) {
      this.simulation = true;
    }
    // Si on peut BREW , on BREW, et on resimulera plus tard
    if (Me.setBrew()) {
      this.action = "BREW";
    ...
    if (this.simulation) {
      ...
      this.path = [...sim.action];
      this.pathTarget = sim.target;
    }

    // On vérifie que le sort de math qu'on doit lancer est lançable
    let spell = undefined;

    Me.spellBook.list.forEach((el) => {
      if (el.id === this.path[0].spell.id) {
        spell = el;
      }
    });

    if (spell.isCastable(Me.stock)) {
      this.action = "CAST";
      this.target = this.path[0].spell.id;
      this.path.shift();
    } else {
      this.action = "REST";
      this.target = "";
